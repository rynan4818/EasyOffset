...
設定値は、基本ゲーム設定やSaberTailorなどとは異なります。
...
- `Adjustment Mode` リストで `Position` を選択します。
- `Pivot point` を手首のピボットポイントに合わせます。コントローラーとの相対的な位置関係はIRL(現実世界)で確認できます。
- `ZOffset`設定を使ってセイバーを軸に沿って移動させることができます。
